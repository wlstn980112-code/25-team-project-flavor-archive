...
const BASE_URL = process.env.NEXT_PUBLIC_MFDS_BASE_URL || 'http://openapi.foodsafetykorea.go.kr/api'
...
export async function getMfdsRecipeList(
  ...
  if (!API_KEY) {
    console.error('🏛️ [MFDS API] 환경 변수가 설정되지 않았습니다.')
    throw new Error('API 키가 설정되지 않았습니다.')
  }

  const url = `${BASE_URL}/${API_KEY}/${SERVICE_ID}/json/${start}/${end}`

  console.log('🏛️ [MFDS API] 레시피 목록 요청:', { start, end, url })
  ...
  try {
    ...
    if (!data.COOKRCP01 || !data.COOKRCP01.row) {
      console.warn('⚠️ [MFDS API] 예상하지 못한 데이터 구조:', data)
      return []
    }
...
export function parseHashTags(recipe: RecipeItem): string[] {
  ...
  const tags = recipe.HASH_TAG.split(/[,\s#]+/)
    ...
    .map((tag) => (tag.startsWith('#') ? tag : tag)) // # 제거하지 않고 그대로 사용
